...
def reslut_record(op_status,data_ip,vip):
    if op_status==0:
        ...
        body='vip:%s,data_ip:%s,cluster information abnormal in the redis' % (vip,data_ip)
        ...
        log('%s mysql is sync failed' %(data_ip,))
        subject='mysql is sync problems,vip:%s' % vip
        body='VIP is %s,data_ip is %s,mysql is sync problems' % (vip,data_ip)
        sendmail('dba@sogou-inc.com','lihui210694@sogou-inc.com',subject,body)